	...
	function checkUser($userData = array()){
        if(!empty($userData)){
            ...
            if($prevResult->num_rows > 0){
				...
				$id=$userData['email'];
				$_SESSION["user_x"]=$id;
				if($_SESSION["user_x"]==$id)
				{
				echo '<script>window.location.href = "../dashboard.php";</script>';	
				}
				else
				{
					//echo '<script>window.location.href = "../form.php?result=Fail";</script>';	
				}
			...
			else
			{
				...
				echo '<script>window.location.href = "../form.php?gmail=yes";</script>';				
            }
